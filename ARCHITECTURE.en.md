...
**Canary wiring status (current release, v0.2+).** `CanaryTokenRegistry`
...
- **`zee restore` authentication** — from v0.3, a local restore_token
  generated by `zee init-restore-token` is required (passed via
  `--token` or `ZEE_RESTORE_TOKEN`; constant-time `hmac.compare_digest`
  match; standard library only). Stops accidental / casual reverts
  from a second shell of the same user. A root-equivalent attacker
  who can read `~/.zee/restore_token` (0600) still bypasses it — for
  multi-user production deployments, wrap `zee restore` in `sudo` or
  run Zee under a dedicated user as a complement.
- **Cut-state persistence** — from v0.3, the interface / service /
  rule names Zee modified at cut time are recorded in
  `~/.local/state/zee/cut_state.jsonl` (0600). Restore reads that
  record and undoes only Zee's changes — the v0.2 side effect of
  re-enabling interfaces other tools had disabled is gone.
- **Locale-independent Windows enumeration** — from v0.3, the primary
  path is PowerShell `Get-NetAdapter` (structured JSON, locale-
  independent). The `netsh interface show interface` parser is kept
  as a fallback.
- **`events.jsonl` no longer carries `decoy_path`** — from v0.3, each
  trap_event record carries `decoy_ref` (`<asset_id>#<index>`)
  instead. An attacker reading the log alone cannot enumerate every
...
- **PAM / FIDO2 for multi-user deployments** — restore_token plus
  hardware-key or PAM auth where same-user-attacker resistance is
  required. The MVP does not bundle this; `sudo`-wrapping `zee
  restore` is the operational substitute.
