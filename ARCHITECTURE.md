...
**v0.1 における canary の配線状況**：`CanaryTokenRegistry`（`src/zee/decoy/canary_token.py`）はトークン発行と登録のための **データ構造のみ** を提供します。v0.1 では seeder からは呼ばれず、囮ファイルに canary URL は自動埋め込みされません。したがって macOS / Windows でのデコイ読み取り検知は v0.1 では発火しません（kqueue / ReadDirectoryChangesW は read を通知せず、canary URL も埋まっていないため）。operator が手動で囮ファイルへ canary URL を埋め込むことは可能ですが、その配線は別タスクで MVP 機能として整備予定です。
...
| 構成要素 | 実装状況 | 場所と備考 |
|---|---|---|
| ① ふるまい観察（囮トリップワイヤ） | ✅ Implemented (change-only on macOS / Windows) | `src/zee/watcher/` — Linux は inotify で open/read/modify を直接観察、macOS は kqueue で変更系のみ、Windows は ReadDirectoryChangesW で変更系のみ。**canary URL での read 検知は v0.1 未配線**（macOS / Windows のデコイ読み取りのみの攻撃は v0.1 では観測されません）。Windows 実機未検証 |
